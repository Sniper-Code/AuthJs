...
class RouterFactory {
    ...
    setUpControllers() {
        if (this.BasePath && this.ControllerType) {
            ...
            if (Controllers.length != 0) {
                Controllers
                    .forEach(ControllerPath => {
                        ...
                        console.log(`🔗 Establishing ${this.ControllerType} Controller [${ControllerName}] at [/${IsAPI ? ApiDir + LcControllerName + "/" : ""}] with ${RouterFactory.FileSystem.path_resolve(ControllerPath).replace(/\\/g, '/')}`)
            ...
            console.error(`🔗 Base Path and Controller type is not set.`);
        return this;
    }
    /**
