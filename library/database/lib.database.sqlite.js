...
class Database extends QueryBuilder {
    ...
    */
    ...
    create() {
        ...
        if (!this.dbFileName) {
            console.log("🐞 Database File Name has not been set");
        } else {
            // Creating Directory if not exists
            if (!fs.dir_exists(Database.Configuration.dbPath)) {
                fs.mkdir(Database.Configuration.dbPath);
                console.log(`📁 Directory Created: ${fs.path_resolve(Database.Configuration.dbPath)}`);
            }
            // Creating Database File if not exists
            if (!fs.file_exists(fs.path_resolve(this.dbPath))) {
                fs.write_file(this.dbPath, '');
                console.log(`💾 Database File Created:  file://${fs.path_resolve(this.dbPath).replace(/\\/g, '/')}`);
            }
        }
        // If File Exists
        if (fs.file_exists(fs.path_resolve(this.dbPath))) {
            ...
            Child
                ...
                .on('data', (data) => console.log(`📺 ${data}`))
                .on('close', (code) => console.log(`📺 Child Process Closed with Code: ${code}`));
        }
        return this;
    }
        ...
        if (fs.file_exists(fs.path_resolve(this.dbPath))) {
            ...
            this.database = new SQLite(this.dbPath, Database.SQLite3.OPEN_READWRITE, (err) => {
                if (err) {
                    ...
                    console.log(`🌎 Database Connected : [${this.dbName}] at ${this.dbPath}`)
                }
            });
        } else
            console.log(`🐞 Database File Not Found: [${this.dbName}] at ${this.dbPath}`);
        return this;
    }
    ...
    disconnectDatabase() {
        ...
        console.log(`🌎 Database Disconnected: [${this.dbName}] at ${this.dbPath}`)
        return this;
    }
    ...
    drop() {
        ...
        if (!this.dbFileName) {
            ...
            if (fs.dir_exists(Database.Configuration.dbPath)) {
                ...
                console.log(`📁 Directory Deleted: ${fs.path_resolve(Database.Configuration.dbPath)}`);
            }
        }
        return this;
    }
    ...
    executeQuery(SQLQuery, callback) {
        ...
        if (SQLQuery.includes(QueryBuilder.statementType.SELECT))
            this.database.all(SQLQuery, (err, rows) => {
                if (err) {
                    console.log(`🐞 ${err.code} ${err} ${err.stack}\n`);
                    callback({
                        error: err,
                        rows: null,
                        status: false
